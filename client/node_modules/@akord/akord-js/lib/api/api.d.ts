...
declare abstract class Api {
    ...
    abstract getUserFromEmail(email: string): Promise<{
        address: string;
        publicKey: string;
    }>;
    abstract uploadFile(file: ArrayBufferLike, tags: Tags, isPublic?: boolean, shouldBundleTransaction?: boolean, progressHook?: (progress: number) => void, cancelHook?: AbortController): Promise<{
        resourceTx: string;
        resourceUrl?: string;
    }>;
    ...
    abstract inviteNewUser(vaultId: string, email: string, role: string): Promise<{
        id: string;
    }>;
